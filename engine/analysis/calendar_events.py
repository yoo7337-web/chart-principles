...
def fetch_kr(probe: bool = False):
    ...
    # 이름 정규화 매칭용 — KIND "현대자동차" vs 유니버스 "현대차"처럼 표기가 다른 경우 보정
    def _norm(s: str) -> str:
    ...
    for nm, cd in name2code.items():
        norm2code.setdefault(_norm(nm), cd)

    # KIND 표기 ≠ 유니버스 표기 별칭(부분일치로는 못 잡는 케이스)
    ALIAS = {"현대자동차": "현대차", "기아자동차": "기아", "네이버": "NAVER",
             "엘지디스플레이": "LG디스플레이", "에스케이하이닉스": "SK하이닉스",
             "삼성전자우선주": "삼성전자우"}

    def _resolve(name: str) -> str:
        for cand in (name, ALIAS.get(name, "")):
            if cand and cand in name2code:
                return name2code[cand]
        n = _norm(ALIAS.get(name) or name)
        if n in norm2code:
            return norm2code[n]
        # 부분일치(한쪽이 다른 쪽을 포함) — 유일할 때만 채택
        cands = [cd for k, cd in norm2code.items()
                 if len(k) >= 3 and (k in n or n in k)]
        return cands[0] if len(cands) == 1 else ""

    out, seen = [], set()
    for tr in re.findall(r"<tr[^>]*>(.*?)</tr>", html, re.S):
        tds = [html_mod.unescape(re.sub(r"<[^>]+>|\s+", " ", td)).strip()
        ...
        seen.add(key)
        code = _resolve(name)
        m = re.match(r"^(\d{1,2}):(\d{2})$", tm or "")
        tm = f"{int(m.group(1)):02d}:{m.group(2)}" if m else tm  # 9:00 → 09:00
