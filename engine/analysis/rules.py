...
BUY = [
    ...
    # v287(2026-08-03) 조건 강화 — 사용자 제보에서 시작한 재점검:
    #   "SK하이닉스에 이격도 신호가 3연속 떴는데 계속 하락"(2026-07-08/16/24, 셋 다 MA120 위·거래량 0.8~1.2배).
    #   전 종목 116,111건으로 재검증하니 **MA120 위에서 난 낙폭은 승률 41.5%·edge20 −1.61%로 손실**이었다.
    #   좋은 표본이 나쁜 표본을 가려 현행(+4.46%)이 통과하고 있었던 것.
    #     현행 −15%만: n=42,940 승률 54.5% edge20 +4.46%
    #     +MA120 아래: n=36,167 승률 56.0% +5.10%
    #     +MA120 아래 & 거래량 1.5배↑: n=14,859 승률 **61.6%** **+7.93%** (전·후반 +12.25/+3.83, 한·미 +8.19/+2.56)
    #   → 마지막 조합을 채택. 신호는 1/3로 줄지만 '떨어지는 칼날'을 걸러낸다.
    Rule("disparity_low", "buy", "이격도 과대낙폭(추세 이탈+투매)",
         "20일선 이격도 -15% 이하 + 종가가 MA120 아래(장기추세 이미 이탈) + 거래량 20일 평균의 1.5배 이상",
