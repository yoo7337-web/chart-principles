...
function lkRangeNote(st, s, tf) {
  ...
  const span = `${from}부터${yrs >= 1.5 ? ` ${Math.round(yrs)}년` : ""} · ${n}`;
  return span + (st.wk_before && tf === "d"
    ? ` <span class="sub-note">(${st.wk_before} 이전은 주봉으로 압축 · 신호 마커는 일봉 구간만)</span>` : "");
}
...
function drawLookupChart() {
  ...
  $("#lookup-info").innerHTML =
    ...
    + (isMin
      ...
      : `기준일 ${st.asof} · ${TF_KO[tf]} · ${lkRangeNote(st, s, tf)} (좌우로 드래그·스크롤)`
...
const DEV_HISTORY = [
  ["v405", "2026-08-13", "🗂 배지 펼침을 카테고리 패널로",
   "배지 펼침 화면이 자동 배치 그리드라 카테고리가 3열에 흩어져 읽기 어려웠습니다(사용자 제보). "
   + "**카테고리별 한 행**(라벨 고정폭 118px + 배지 목록)의 정돈된 패널로 재구성 — 산업·밸류체인 / "
   + "자본 이벤트 / 내부자 매매 / 예측 vs 실적 / 차트·괴리 신호 / 보고서·심층 / 심층 정보 순서로 "
   + "라벨이 세로로 정렬됩니다. 좁은 화면(700px↓)에선 라벨이 위로 접힙니다."],
  ...
  ["v392", "2026-08-13", "💰 자본 이벤트 신설 — 자사주(환원) vs 유증·CB(희석)",
   ...
   + "금액·시총 대비 비율에 **유증 희석률**(신주÷기존주식수)·**CB 전환가**까지 구조화 필드 그대로입니다.\n\n"
   + "화면: ①환원/희석·유형별 필터 ②**환원 강도·희석 규모 랭킹**(시총 대비 %) ③**종목조회 헤더 배지**"
   + "(그 종목의 최근 90일 이벤트) ④공시 스캐너에 **카테고리 54일 추세**(급증 감지)와 **👤 내 종목만 필터**"
   + "(보유 💼·관심 ⭐ 배지)를 함께 넣었습니다.\n\n"
   + "수집은 공시 스캐너가 이미 쌓는 일일 공시에서 대상만 골라 조회하는 증분 방식이라 하루 10~30콜입니다."],
  ...
  ["v387", "2026-08-08", "🎨 재무 필터 디자인 정리 · 순환성 글자 크기 축소",
   ...
   + "**색 악센트** ③적용 개수를 **알약 배지**로 ④칩을 조금 작게, 선택 상태는 더 또렷하게 "
   + "⑤행 사이 얇은 구분선.\n\n"
   + "⚠수급 카드의 검증표(6열)가 카드를 줄어들지 못하게 해 **가로 스크롤이 생기고 있었습니다** — "
   + "표는 자기 영역에서만 스크롤하도록 고쳤습니다. 화면 폭에 따라 4→3→2→1열로 접힙니다.\n\n"
   + "**종목조회 순환성** 블록은 넓은 카드용 스타일을 쓰던 탓에 좁은 오른쪽 레일에서 글자가 컸고 "
   + "라벨이 줄바꿈됐습니다('영업이익 증감 변/동') → 옆 투자지표와 같은 크기로 맞추고 라벨 칸을 넓혀 "
   + "한 줄에 들어가게 했습니다."],
  ["v386", "2026-08-08", "🐞 거래정지 종목 차트에 바닥까지 뻗는 빨간 막대",
   "제보: 거래가 없는 날에 **화면을 가로지르는 긴 빨간 막대**가 생긴다. 원인은 국내 시세 소스가 "
   + "거래 없는 날을 **시가·고가·저가 = 0, 종가 = 전일가**로 주기 때문이었습니다 — 캔들이 0에서 "
   + "종가까지 그려집니다.\n\n"
   + "거래가 없었으면 가격 변동도 없으므로 **시가·고가·저가를 종가로 맞춰** 평평한 봉(점처럼)이 "
   + "되게 했습니다. 실측 **749종목·74,362행**이 해당됐고, 쌓인 데이터도 모두 소급 보정했습니다.\n\n"
   + "덤으로 이 0값은 **지표까지 오염**시키고 있었습니다(저가 0이 고저 기반 계산에 그대로 들어감) — "
   + "이제 수집 단계와 로드 단계 양쪽에서 막습니다."],
  ...
  ["v384", "2026-08-08", "🐞 종목조회 이동 링크 4곳이 죽어 있던 문제",
   ...
   + "종목 조회를 **메뉴에서 빼고 제목줄 검색창으로 옮긴 뒤**(v277) 그 버튼이 없어져 클릭할 대상이 "
   + "사라졌습니다. 그 순간 오류가 나면서 이후 코드(종목 로드)가 실행되지 않았습니다.\n\n"
   + "같은 방식을 쓰던 **4곳이 모두 무반응**이었습니다 — 오늘의 신호 표·요약 타일, 보유 매매이력, "
   + "투자 대가 편입·처분 카드. 이동 경로를 `gotoLookup()` 한 곳으로 모아 고쳤고, 같은 일을 하던 "
   + "다른 7곳도 이 함수로 통일해 앞으로 자리가 또 바뀌어도 한 곳만 고치면 되게 했습니다."],
  ...
  ["v380", "2026-08-08", "📈 종목 차트 전기간으로 — 미국은 1962년까지",
   ...
   + "**3,000행이 상한**이라 2014년 5월까지가 최대치이고, 그 전 구간은 어떤 방법으로도 받을 수 없습니다.\n\n"
   + "⚠**용량 절충**: 봉당 약 73바이트라 전기간을 일봉으로 담으면 게시 사이트가 612MB → 845MB가 되어 "
   + "GitHub Pages 상한(1GB)에 여유가 없어집니다. 그래서 **최근 10년은 일봉 원본, 그보다 과거는 주봉**으로 "
   + "압축했습니다(한·미 동일). 삼성전자는 2015-12-30 이전이, J&J는 2016-04-05 이전이 주봉입니다 — "
   + "차트 안내줄에 그 경계를 표시합니다.\n\n"
   + "원칙 성적(승률·초과수익)은 원래 전체 일봉으로 계산하므로 영향이 없고, 신호 마커는 일봉 구간에만 붙습니다."],
  ...
  ["v378", "2026-08-08", "주식찾기 레이아웃 — 필터를 상단으로, 표를 전폭으로",
   ...
   + "덕분에 **결과 표가 화면 오른쪽 끝까지** 쓰고(1,047 → 1,377px), 세부 지표도 좁은 사이드에서 한 줄씩 쌓이던 것이 "
   + "4열로 넓게 펼쳐져 한눈에 보입니다."],
  ...
  ["v347", "2026-08-07", "데이터 갱신 기준표 · 갱신 파이프라인 안정화",
   ...
   + "보이지 않는 곳도 정비했습니다: ①손상된 가격 캐시 하나가 마켓 갱신 전체를 24시간 멈추게 한 버그를 고치고 "
   + "재발하지 않도록 저장 방식을 바꿨습니다 ②하루 1회 52분짜리 종목 피드 수집이 30분 갱신 주기를 밀어내던 것을 "
   + "새벽 6시 전용 일정으로 분리했습니다 — 마켓 데이터가 더 일정하게 30분마다 갱신됩니다."],
  ...
  ["v317", "2026-08-04", "보고서 버튼 정렬·색 구분 · 재무 X축 정렬",
   ...
   + "관심종목 재무 그래프의 **X축이 데이터 위치와 어긋나던 문제**를 고쳤습니다(축 그림이 그래프와 폭이 달랐습니다). "
   + "'적전'처럼 증감률 글자가 잘리던 것도 해결했습니다."],
...
function renderValueBand(st) {
  ...
  const nowPx = freshQuote(st)?.cur ?? now.c;   // ⚠반환 스키마는 {cur,chg,src} — .price는 v315와 같은 오독
